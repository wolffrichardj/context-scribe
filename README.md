...
1.  **Observer**: A robust `watchdog` powered monitor with manual periodic scanning ensuring zero missed events across supported JSON log streams and Antigravity SQLite chat state.
...
> [!IMPORTANT]
> **Currently, [Gemini CLI](https://github.com/google/gemini-cli) and Antigravity watchers are supported.**
> 
> Context-Scribe now watches Gemini JSON logs directly and Antigravity chat state from the local `state.vscdb` SQLite index in the app workspace storage. The evaluator still runs through the local Gemini CLI, while the observer can be selected with `--tool`.
...
To watch Antigravity chat state instead, switch the tool value. By default Context-Scribe scans the Antigravity `workspaceStorage/**/state.vscdb` SQLite files (for example `~/.config/Antigravity/User/workspaceStorage/` on Linux):
```bash
context-scribe --tool antigravity
```
...
1.  **Start the daemon**: `context-scribe --tool gemini` (or `context-scribe --tool antigravity`)
...
2.  **Simulate an interaction**: 
    ...
    # or, for Antigravity watchers, update the workspaceStorage state.vscdb entry keyed by
    # workbench.panel.aichat.view.aichat.chatdata inside your Antigravity workspace storage.
    ```
